...
///
/// # Errors
/// `SymlinkTooLong` if `out` is too small for the UTF-8 expansion.
///
/// # Notes
...
///
/// # Errors
/// `SymlinkTooLong` if `out` is too small.
pub fn read_symlink_target_with_block_size(
    buf: &[u8],
    block_size: usize,
    out: &mut [u8],
) -> Result<usize, crate::AffsError> {
...
/// # Errors
/// `SymlinkTooLong` if `out` is too small.
fn latin1_to_utf8_symlink(latin1: &[u8], out: &mut [u8]) -> Result<usize, crate::AffsError> {
    let mut out_pos = 0;
    ...
    for (i, &byte) in latin1.iter().enumerate() {
        ...
        if byte < 0x80 {
            // ASCII - direct copy
            if out_pos >= out.len() {
                return Err(crate::AffsError::SymlinkTooLong);
            }
            out[out_pos] = byte;
            out_pos += 1;
        } else {
...
mod tests {
    ...
    fn test_latin1_to_utf8_ascii() {
        let input = b"hello";
        let mut out = [0u8; 32];
        let len = latin1_to_utf8_symlink(input, &mut out).unwrap();
        assert_eq!(len, 5);
    ...
    #[test]
    fn test_symlink_too_long() {
        let input = b"hello";
        let mut out = [0u8; 2]; // too small
        let res = latin1_to_utf8_symlink(input, &mut out);
        assert!(matches!(res, Err(crate::AffsError::SymlinkTooLong)));
    }
}
